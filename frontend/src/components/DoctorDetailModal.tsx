...
export function DoctorDetailModal({ doctor, onClose }: DoctorDetailModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-slate-100 overflow-hidden">
        {/* Header */}
        ...
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="flex items-start gap-3">
            <svg
              className="h-5 w-5 text-slate-400 mt-0.5 shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
          ...
          {doctor.google_rating && (
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-slate-700">
                {doctor.google_rating} / 5
              </span>
              {doctor.google_review_count != null && (
                <span className="text-xs text-slate-400">
                  ({doctor.google_review_count} reviews)
                </span>
              )}
            </div>
          )}
        ...
        <div className="border-t border-slate-100 px-6 py-4 flex gap-3">
          {doctor.phone && (
            <a
              href={`tel:${doctor.phone}`}
              className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-indigo-600 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition-colors"
            >
              <svg
                className="h-4 w-4"
                fill="none"
          ...
          {doctor.profile_url && (
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-white py-2.5 text-sm font-semibold text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50 transition-colors"
            >
              <svg
                className="h-4 w-4"
                fill="none"
